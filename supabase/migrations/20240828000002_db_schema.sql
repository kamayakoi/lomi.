...
CREATE INDEX idx_merchant_org_links_invitation_email ON merchant_organization_links(invitation_email);
CREATE INDEX idx_merchant_org_links_status ON merchant_organization_links(team_status);
CREATE INDEX idx_merchant_org_links_role ON merchant_organization_links(role);
...
-- Create a unique index to ensure only one active admin per organization
CREATE UNIQUE INDEX idx_single_active_admin_per_org 
ON merchant_organization_links (organization_id) 
WHERE role = 'Admin' AND team_status = 'active';

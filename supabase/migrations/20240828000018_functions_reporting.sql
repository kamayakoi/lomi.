...
-- Function to fetch MRR data for the chart with continuous data points
...
RETURNS TABLE (
    ...
    expansion_mrr NUMERIC(15,2),
    contraction_mrr NUMERIC(15,2)
) AS $$
DECLARE
    v_interval interval;
    v_date_trunc text;
BEGIN
    -- Calculate the interval based on the date range
    IF p_end_date - p_start_date <= interval '7 days' THEN
        v_interval := '1 day'::interval;
        v_date_trunc := 'day';
    ELSE
        v_interval := '1 day'::interval;
    ...
    subscription_changes AS (
        SELECT 
            date_trunc(v_date_trunc, COALESCE(ms.updated_at, ms.created_at)) as change_date,
            CASE 
                WHEN sp.billing_frequency = 'monthly' THEN sp.amount
                WHEN sp.billing_frequency = 'yearly' THEN sp.amount / 12
                WHEN sp.billing_frequency = 'quarterly' THEN sp.amount / 3
            ...
            ms.status,
            LAG(sp.amount) OVER (PARTITION BY ms.customer_id ORDER BY ms.updated_at) as previous_amount,
            ms.created_at = ms.start_date as is_new
        FROM merchant_subscriptions ms
        JOIN subscription_plans sp ON ms.plan_id = sp.plan_id
        WHERE ms.merchant_id = p_merchant_id
        AND COALESCE(ms.updated_at, ms.created_at) BETWEEN p_start_date AND p_end_date
    )
    SELECT 
        t.timepoint as date,
        COALESCE(SUM(
            CASE WHEN sc.status = 'active' THEN sc.monthly_amount ELSE 0 END
        ) OVER (ORDER BY t.timepoint), 0) as mrr,
        COALESCE(SUM(
            CASE WHEN sc.is_new AND sc.status = 'active' THEN sc.monthly_amount ELSE 0 END
        ), 0) as new_mrr,
        COALESCE(SUM(
            CASE WHEN sc.status = 'cancelled' THEN sc.monthly_amount ELSE 0 END
        ), 0) as churned_mrr,
        COALESCE(SUM(
            CASE 
                WHEN NOT sc.is_new AND sc.status = 'active' 
                AND sc.monthly_amount > COALESCE(sc.previous_amount, 0) 
                THEN sc.monthly_amount - COALESCE(sc.previous_amount, 0)
                ELSE 0 
            END
        ), 0) as expansion_mrr,
        COALESCE(SUM(
            CASE 
                WHEN NOT sc.is_new AND sc.status = 'active' 
                AND sc.monthly_amount < COALESCE(sc.previous_amount, 0) 
                THEN COALESCE(sc.previous_amount, 0) - sc.monthly_amount
                ELSE 0 
            END
        ), 0) as contraction_mrr
    FROM timepoints t
    LEFT JOIN subscription_changes sc ON date_trunc(v_date_trunc, sc.change_date) = t.timepoint

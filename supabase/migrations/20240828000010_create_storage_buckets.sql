...
-- Create a function to check image dimensions and size for avatars
CREATE OR REPLACE FUNCTION check_avatar_dimensions()
RETURNS TRIGGER AS $$
DECLARE
    file_size INT;
    width INT;
...
-- Create a trigger to run the check function for avatars
CREATE TRIGGER check_avatar_dimensions_trigger
AFTER INSERT OR UPDATE ON storage.objects
FOR EACH ROW
WHEN (NEW.bucket_id = 'avatars')
EXECUTE FUNCTION check_avatar_dimensions();

-- Create a function to check image dimensions and size for logos
...
-- Create a trigger to run the check function for logos

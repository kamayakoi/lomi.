...
interface NewOrganizationLogoUploaderProps {
    currentLogo: string | null
    onLogoUpdate: (newLogoUrl: string) => void
    companyName: string
}

export default function NewOrganizationLogoUploader({ currentLogo, onLogoUpdate, companyName }: NewOrganizationLogoUploaderProps) {
    ...
    const handleSave = async () => {
        if (selectedFile && croppedAreaPixels) {
            try {
                ...
                if (croppedImage) {
                    const previewBlobUrl = URL.createObjectURL(croppedImage)
                    setPreviewUrl(previewBlobUrl)
                    onLogoUpdate(previewBlobUrl)
                }
            } catch (error) {
    ...
    const handleRemove = () => {
        ...
        const fileInput = document.getElementById('logo-upload-new') as HTMLInputElement
        if (fileInput) {
            fileInput.value = ''
        }
    }
            ...
            <div className="flex items-center space-x-4">
                ...
                <div className="space-x-2">
                    ...
                    <Button
                        ...
                        onClick={(e) => {
                            e.preventDefault();
                            document.getElementById('logo-upload-new')?.click();
                        }}

        ...
        >
            <div className="container mx-auto px-4 py-0 max-w-7xl">
                ...
                {loading ? (
                    ...
                    >
                        <h2 className="text-2xl font-semibold mb-4 dark:text-white">{t("blog.noPosts")}</h2>
                        <p className="text-zinc-600 dark:text-zinc-400 mb-6">{t("blog.noPostsDesc")}</p>
                        <button
                            onClick={() => navigate('/')}
                                        ...
                                        <article className="flex flex-col h-[410px] sm:h-auto bg-white dark:bg-zinc-900 rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-zinc-200 dark:border-zinc-800" style={{ height: "calc(100% + 15px)" }}>
                                            {post.image && (
                                                <div className="aspect-[3/2] overflow-hidden border-b border-zinc-200 dark:border-zinc-800">
                                                    <img
                                                        ...
                                                        alt={post.image.alt || getLocalizedField(post, 'title', post.title)}
                                            ...
                                            <div className="p-4 flex flex-col flex-grow">
                                                ...
                                                <div className="mt-auto flex flex-row justify-between items-center">
                                                    <span className={`px-2 py-1 text-xs font-medium rounded ${getCategoryColor(getPostCategory(post))}`}>
                                                        {t(`blog.categories.${getPostCategory(post)}`)}
                                                    </span>

                                                    <div className="flex items-center text-primary text-sm font-medium">
                                                        <span className="transition-transform duration-300" style={{ transform: hoveredCard === post._id ? 'translateX(-2px)' : 'translateX(0)' }}>
                                                            {t("blog.readArticle")}
                                                        </span>
                                                        <ArrowRight className={`ml-1 h-4 w-4 transition-all duration-300 ${hoveredCard === post._id ? 'opacity-100 translate-x-1' : 'opacity-0 -translate-x-3'}`} />
                                                    </div>
                                                </div>
                                            </div>
                                        </article>

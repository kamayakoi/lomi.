...
- **Secret Keys** (`lomi_sk_test_…` / `lomi_sk_live_…`): Full access, server-side only. **Never share these.**
- **Publishable Keys** (`pk_…` where applicable): Limited access, safe for client-side use only when documented for your integration.

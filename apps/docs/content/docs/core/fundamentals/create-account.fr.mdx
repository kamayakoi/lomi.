...
Créez un fichier `.env` (voir [Variables d’environnement](/core/fundamentals/access-tokens)). En phase de test, utilisez des clés **test** — le SDK et l’API vous limitent au bac à sable :
...
| Clé | Préfixe | Usage |
| --- | --- | --- |
| Secrète | `lomi_sk_test_…` / `lomi_sk_live_…` | Serveur uniquement |
| Publiable | `lomi_pk_test_…` / `lomi_pk_live_…` | Navigateur / mobile (flux carte) |

Passez à `lomi_sk_live_…` / `lomi_pk_live_…` au passage en production.
...
// Initialize the SDK (« test » pour le bac à sable pendant le développement)

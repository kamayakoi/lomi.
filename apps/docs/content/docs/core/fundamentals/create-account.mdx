...
| ------------------- | ----------------------------------------- | ------------------------------------- |
| **Publishable key** | `lomi_pk_test_...` / `lomi_pk_live_...` | Client-side safe. Limited operations. |
| **Secret key**      | `lomi_sk_test_...` / `lomi_sk_live_...` | Server-side only. Full API access.    |
...
Create a `.env` file in your project (see [Environment variables](/core/fundamentals/access-tokens)). While testing, use **test** keys — the SDK and API scope you to the sandbox automatically:
...
| Key | Prefix | Use on |
| --- | --- | --- |
| Secret | `lomi_sk_test_…` / `lomi_sk_live_…` | Server only |
| Publishable | `lomi_pk_test_…` / `lomi_pk_live_…` | Browser / mobile (card flows only) |

Switch to `lomi_sk_live_…` / `lomi_pk_live_…` when you go live.
...
// Initialize the SDK (use 'test' for sandbox while building)
...
- [Manage access tokens](/core/fundamentals/access-tokens) - Understand secret vs publishable keys
- [Configure webhooks](/core/fundamentals/webhooks) - Handle payment events
- [Process transactions](/core/fundamentals/transactions) - View and manage payments

...
<Callout type="info">
  For retries, duplicate delivery, and delivery-log debugging, see [Webhook reliability](/core/advanced-guides/webhook-reliability).
</Callout>
...
Each delivery attempt uses a bounded HTTP read timeout (**about four seconds**) on lomi.’s outbound client. Waiting until that limit means the attempt is recorded as unsuccessful for that round. You should comfortably stay inside that bound by answering with **`200`** or **`204`** in normal conditions (typically well under **one second**), then enqueue any heavy lifting. Operational numbers, backoff, and when lomi retries are spelled out under [Webhook reliability — Timeouts and retries](/core/advanced-guides/webhook-reliability#timeouts-retries-and-client-errors).
...
For tables, environment flags, and implementation-oriented detail, read **[Webhook reliability — Timeouts and retries](/core/advanced-guides/webhook-reliability#timeouts-retries-and-client-errors)** — that page is the canonical reference for how the API service behaves today.
...
- **Respond Quickly:** Send a `200 OK` (or other `2xx`) as soon as the signature checks out — well **before** lomi’s outbound **~4 second** read timer becomes a factor for that attempt ([why](/core/advanced-guides/webhook-reliability#timeouts-retries-and-client-errors)).
...
- **Secure Your Secret:** Keep your webhook signing secret confidential. Do not commit it to version control. Use environment variables or a secrets manager (see [API Keys guide](/core/fundamentals/access-tokens)).
...
- **Wrong endpoint path (very common):** If your webhook URL looks like a frontend page or redirect destination (for example `` `/pricing?canceled=1` ``, `` `/checkout/success` ``, or any route configured as checkout `` `cancel_url` `` / `` `success_url` ``), deliveries will fail. Use a dedicated server endpoint such as `` `/api/lomi/webhook` ``.
- **401 Unauthorized / `"Authentication required"` in the response body:** That message is normally **from your endpoint or reverse proxy**, not from lomi. Outbound webhook POSTs include `X-Lomi-Signature` for verification only — **not** `Authorization: Bearer` or `X-API-Key`. Exclude your webhook route from blanket API authentication and verify the HMAC ; see [Signing secret vs Authorization headers](/core/advanced-guides/handling-webhooks#webhook-signing-vs-authorization).

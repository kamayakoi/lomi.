...
Voir aussi [les journaux de livraison](/reference/platform/webhook-delivery-logs) : lomi. enregistre le code HTTP et le corps renvoyé par **votre** point de terminaison.
...
Répondez en **quelques secondes tout au plus** dans les cas extrêmes, mais visez **sous une seconde** en usage courant : chaque envoi HTTP sortant côté lomi est cadencé par un timeout de lecture **d’environ quatre secondes** ; le dépasser fait échouer la tentative et peut consommer une partie de vos relances automatiques ([matrice détaillée](/core/advanced-guides/webhook-reliability#timeouts-retries-and-client-errors)). Le travail lourd passe ensuite dans **votre** file interne.

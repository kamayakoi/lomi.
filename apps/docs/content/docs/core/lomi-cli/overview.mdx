...
Opens a browser device-authorization flow and saves a CLI token globally. See [Authenticate](/core/lomi-cli/auth).
...
Creates SDK client setup, checkout and webhook examples, and a `.env` file. See [Initialize a project](/core/lomi-cli/init).
...
Installs Cursor rules, `CLAUDE.md`, `AGENTS.md`, VS Code instructions, and `llms.txt`. See [Install agent rules](/core/lomi-cli/install-rules).
...
Starts a webhook receiver on `http://localhost:4242/webhook`. See [Local development server](/core/lomi-cli/dev).
...
Full reference: [Command reference](/core/lomi-cli/commands).
...
- [Authenticate](/core/lomi-cli/auth) — device flow, profiles, CI
- [Initialize a project](/core/lomi-cli/init) — SDK scaffolding
- [Command reference](/core/lomi-cli/commands) — all commands and flags
- [Configuration](/core/lomi-cli/configuration) — config file paths and environment variables
- [API integration](/reference/setup/integration) — HTTP and SDK integration guide

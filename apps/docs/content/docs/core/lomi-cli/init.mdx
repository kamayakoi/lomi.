...
Requires prior [`lomi login`](/core/lomi-cli/auth) (or `LOMI_ACCESS_TOKEN` in CI).
...
| `--skip-rules-install` | Skip the agent rules install prompt |
...
It may also offer to run [`lomi install-rules`](/core/lomi-cli/install-rules) if you haven't installed agent rules yet.
...
Point your dashboard webhook URL to a tunnel (e.g. ngrok) or use the local server during development. See [Local development server](/core/lomi-cli/dev).
...
- [`lomi login`](/core/lomi-cli/auth) completed (or `LOMI_ACCESS_TOKEN` set)
- A **secret API key** from the Merchant Portal (`lomi_sk_test_…` or `lomi_sk_live_…`)

## Next steps

- [Local development server](/core/lomi-cli/dev)

...
Most **lomi. CLI** commands (like `payments`, `webhook`, `status`, `dev`) need permission to interact with the **lomi. API**. This authentication is managed globally using the [`lomi. login`](/core/lomi-cli/auth) command.
...
When you initialize a project using [`lomi. init`](/core/lomi-cli/init), it sets up configuration specifically for the **lomi. Node.js SDK** to use _within your application code_.
...
- Learn about [Webhook development](/core/advanced-guides/handling-webhooks)
- Explore our [Testing Guide](/core/advanced-guides/testing)
- Set up [CI/CD Integration](/core/advanced-guides/ci-cd)

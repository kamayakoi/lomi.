---
title: "Créer un remboursement"
description: "Créer un remboursement"
full: true
method: post
path: /refunds
operationId: RefundsController_create
---

## Aperçu

Créer un remboursement

Rembourse une transaction terminée (carte ou mobile money). Le solde marchand est mis à jour immédiatement. Prise en charge des remboursements totaux et partiels.

Pour les paiements liés à un abonnement, utilisez `subscription_action` (optionnel) : `default` (annulation au remboursement total initial, pause au renouvellement), `cancel`, `pause` ou `none`. Un remboursement partiel ne modifie l’abonnement que si le cumul atteint le montant total de la transaction.


### Quand utiliser cet endpoint

Utilisez cet endpoint quand vous devez exécuter `POST /refunds` (Créer un remboursement).


## Authentification

Les routes marchandes nécessitent une clé API dans l’en-tête `X-API-KEY` (voir [Aperçu de l’intégration](/api)). Utilisez une clé **test** avec `https://sandbox.api.lomi.africa` et une clé **live** avec `https://api.lomi.africa`.

## Point de terminaison

`POST /refunds`

URLs de base :

- `https://sandbox.api.lomi.africa`
- `https://api.lomi.africa`

## Requête

### Paramètres de chemin

_Aucun paramètre de chemin au-delà du motif d’URL._


### Paramètres de requête

_Aucun paramètre de requête._


### Corps de la requête

_Aucun corps application/json pour cette opération._


## Réponses

| Statut | Description |
| --- | --- |
| `201` | Remboursement enregistré |
| `400` | Entrée invalide ou type non pris en charge |

## Erreurs

Les erreurs suivent le format JSON standard (code de statut et message lisible par machine). Validez les entrées avant appel ; **401** indique une clé manquante/invalide, **404** une ressource introuvable pour cette organisation, **429** une limitation de débit. Pour les retries sûrs sur les créations, envoyez une clé d’idempotence quand votre flux le permet.

## Exemple

```bash
curl -sS -X POST "https://sandbox.api.lomi.africa/refunds" \
  -H "X-API-KEY: $LOMI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```

## OpenAPI

- **operationId**: `RefundsController_create`
- **Operation**: `POST /refunds`

Schémas complets et **Try it** : [référence API](/api/index). Contrat machine : `openapi.json` à la racine des docs.

...
2. Copier une **clé secrète de test** dans **Paramètres → Jetons d’accès** (`lomi_sk_test_…`). Voir [Variables d’environnement](/core/fundamentals/access-tokens).
3. L’ajouter dans un fichier `.env` (voir [Gestion des variables](/core/fundamentals/access-tokens#gestion-des-variables-denvironnement)).
...
Résultat attendu : liste JSON depuis `GET https://sandbox.api.lomi.africa/accounts`. Sans SDK : [Intégration API](/reference/setup/integration).

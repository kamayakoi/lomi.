...
| `` `first_payment_type` ``    | `` `string` ``  | Type of the first payment: `` `initial` ``, `` `non_initial` ``, or `` `prorated` `` (nullable).               |
...
| `` `amount` ``         | `` `number` ``         | Amount in smallest currency unit. For PWYW, suggested unit price.           |
...
- **API contract:** the `GET /organizations` response shape is defined in the API schema (`` `OrganizationResponseDto` ``); treat the [API reference](/api/index) as canonical for which fields are returned on each endpoint.

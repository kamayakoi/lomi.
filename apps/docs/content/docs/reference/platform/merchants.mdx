...
Requests require authentication using your API key in the `` `X-API-Key` `` header. See the [Authentication](/reference/setup/authentication) guide.

For organization-level settings, pricing, and metrics, prefer the [Organizations](/reference/platform/organizations) API when your integration key is org-scoped.
...
_(See [Data Models](/reference/reference/data-models#merchant-object) for property descriptions)_
...
- `` `merchant_lifetime_value` `` is computed as **average customer LTV × total customers** for the linked organization (live environment).
...
- See the [Errors](/reference/reference/errors) guide for general error handling information.

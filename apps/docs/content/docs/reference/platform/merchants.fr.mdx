...
Les requêtes s’authentifient avec la clé API dans l’en-tête `` `X-API-Key` ``. Voir [Authentification](/reference/setup/authentication).

Pour réglages, tarification et indicateurs au niveau organisation, privilégiez l’API [Organisations](/reference/platform/organizations) lorsque votre clé porte sur l’organisation.
...
_(Voir [Modèles de données](/reference/reference/data-models#merchant-object) pour la description des propriétés.)_
...
- Voir [Erreurs](/reference/reference/errors) pour la gestion d’erreurs générale.

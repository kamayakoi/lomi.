...
<Callout type="warning">
  `` `success_url` `` / `` `cancel_url` `` are customer redirect URLs, **not** webhook URLs.  
  Configure webhook deliveries separately under **Developers → Webhooks** using a backend endpoint (for example `` `https://your-domain.com/api/lomi/webhook` ``).
</Callout>
...
| Status | Description |
|--------|-------------|
| `400` | Invalid input (e.g. amount on a product link) |

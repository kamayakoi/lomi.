...
| Type | Description |
|------|-------------|
| `product` | Links to a specific product. Amount is calculated from product price. **Recurring products** create a subscription when checkout completes. |
| `instant` | Custom amount payment. Requires `amount` field. |

<Callout type="info">
  Product links for **`product_type: recurring`** open hosted checkout in subscription mode (trials, first payment types, and renewals follow [Checkout behavior — Subscription checkout](/reference/payments/checkout-behavior#subscription-checkout)). Instant links are for one-time amounts only.
</Callout>

<Callout type="info">
  **Pay-what-you-want** products use **single-product** links only. Set **`price_id`** when the product has multiple prices; checkout opens with that price’s suggested amount and lets the buyer adjust within `minimum_amount` / `maximum_amount`. Multi-item payment links cannot include PWYW products.
</Callout>

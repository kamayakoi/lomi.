...
<Callout type="info">
  Pour les parcours d’inscription (essais, types de premier paiement, montants de session), voir **[Comportement du tunnel — Abonnements](/reference/payments/checkout-behavior#abonnements)**.
</Callout>
...
**Essais** (`trial_enabled` + `trial_period_days`) : montant à l’inscription **0**. Stripe enregistre la carte via SetupIntent ; le mobile money peut finaliser sans paiement immédiat. Après l’essai, le statut passe à `active` et la facturation démarre à `next_billing_date`.

**Renouvellements** automatiques avec moyen Stripe enregistré. Sans moyen (fréquent en mobile money), lomi. peut basculer vers un **lien de renouvellement manuel** pour le client.
...
Les réponses API peuvent utiliser `subscription_id` plutôt que `id`. Voir **[Abonnements clients](/reference/platform/customer-subscriptions)** pour la forme de liste canonique.

---

## Webhooks

Abonnez-vous à ces événements en **SCREAMING_SNAKE_CASE** (comme sur [Webhooks](/reference/payments/webhooks)) :

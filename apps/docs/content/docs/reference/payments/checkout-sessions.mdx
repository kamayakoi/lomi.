...
<Callout type="warning">
  **Do not use checkout redirects as webhook endpoints.** `` `success_url` `` and `` `cancel_url` `` are browser redirect destinations for your customer.  
  Webhooks must point to a dedicated server route like `` `https://your-domain.com/api/lomi/webhook` `` that accepts unauthenticated `` `POST` `` requests and verifies `` `X-Lomi-Signature` ``.
</Callout>

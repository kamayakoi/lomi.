...
2. **Outbound merchant webhooks** (lomi. calls **your** URL): Subscriptions are configured with the Postgres enum **`webhook_event`**, the same set documented in [Webhooks](/reference/payments/webhooks). The HTTP body field `` `event` `` and the `` `X-Lomi-Event` `` header use those values (for example `` `PAYMENT_SUCCEEDED` ``), as built in `` `WebhookSenderService.prepareWebhookPayload` `` in `` `apps/api` ``.

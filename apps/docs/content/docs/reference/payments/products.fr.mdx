...
<Callout type="info">
  Pour `pay_what_you_want` : `minimum_amount` est obligatoire ; `amount` est le prix unitaire suggéré (par défaut `minimum_amount` si omis). Vérifiez `amount >= minimum_amount`, et si `maximum_amount` est défini, `amount <= maximum_amount`. Pour `standard` et `tiered`, omittez `minimum_amount` et `maximum_amount`.
</Callout>

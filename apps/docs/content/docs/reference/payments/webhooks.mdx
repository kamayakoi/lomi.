...
API requests to manage webhooks require authentication using an API key in the `` `X-API-Key` `` header. See [Authentication](/reference/setup/authentication).
...
**Outbound delivery:** Requests from lomi. to **your** URL are **not** authenticated with `` `Authorization` `` / `` `X-API-Key` `` — only signature verification applies. Putting the signing secret in an `` `Authorization` `` header on your expectation side will not help; middleware that requires Bearer or API-key auth on **every** POST will typically return `` `401` `` ([details](/core/advanced-guides/handling-webhooks#webhook-signing-vs-authorization)).

Refer to the [Webhook Signature Verification](/core/advanced-guides/handling-webhooks) guide for detailed implementation examples in various languages.
...
<Callout type="info">
  **Renewal payment failures** emit **`PAYMENT_FAILED`** on the failed transaction. There is no separate subscription payment-failed webhook. Use **`SUBSCRIPTION_RENEWED`** for successful billing cycles. See [Subscriptions](/reference/payments/subscriptions) and [Checkout behavior — Subscription checkout](/reference/payments/checkout-behavior#subscription-checkout).
</Callout>
...
For operations guidance (retries, idempotency, delivery logs), see [Webhook reliability](/core/advanced-guides/webhook-reliability).
...
Roughly speaking: **`4xx` client errors are usually terminal** for that automated sequence (misconfiguration should be corrected on your side), while **transient failures** (`5xx`, timeouts, certain network faults) may be retried up to a ceiling that depends on whether the webhook was dispatched **synchronously** or through the **queued** worker/outbox pipeline. Older documentation sometimes overstated calendar duration; treat the authoritative breakdown as **[Webhook reliability — Timeouts and retries](/core/advanced-guides/webhook-reliability#timeouts-retries-and-client-errors)**, where we document the approximate **~4 second** outbound HTTP timer, **four** synchronous attempts at most on retryable errors, **five** queue attempts where Bull is in play, and the meaning of **`lomi_environment`**.

## Best practices

- **Route without global API auth:** Apply `X-Lomi-Signature` verification on a dedicated webhook path; do not require `Authorization` / `X-API-Key` on the same POST ([why](/core/advanced-guides/handling-webhooks#webhook-signing-vs-authorization)).
- **Verify signatures:** Always verify the `` `X-Lomi-Signature` `` to ensure the request is from lomi.
- **Respond quickly:** Return `` `2xx` `` promptly — lomi waits only **about 4 seconds** per POST attempt before timing out. Offload heavier work after you respond.

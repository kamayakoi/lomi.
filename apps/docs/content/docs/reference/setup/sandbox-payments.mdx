...
<Mermaid
  chart={`flowchart TD
  apiKey["Test or live API key"] --> env["environment: test or live"]
  link["Payment link or checkout session"] --> env
  env --> ledger["Test balance vs live accounts"]
  env --> method{"Payment method"}
  method --> cards["Card payments"]
  method --> wave["Wave"]
  method --> mtn["MTN MoMo"]`}
/>

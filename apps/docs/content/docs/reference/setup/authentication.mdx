...
For a guided path from your first `curl` to the OpenAPI pages, SDKs, and webhook caveats, see **[API integration](/reference/setup/integration)**.
...
When `` `Lomi-Account` `` is absent, requests behave like normal merchant API calls. When it is present, the API key organization is the Operator and the `` `acct_...` `` account is the target Member Account. See **[lomi. Network](/reference/network)** for supported endpoints and required capabilities.
...
To simulate card and mobile money payments (test card numbers, Wave, MTN MoMo), see **[Sandbox payments](/reference/setup/sandbox-payments)**.

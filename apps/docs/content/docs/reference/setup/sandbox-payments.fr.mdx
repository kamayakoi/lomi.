---
title: 'Paiements en bac à sable'
description: 'Guide complet pour simuler les paiements carte et mobile money dans l’environnement de test lomi.—numéros par marque et scénario, Wave et MTN MoMo, soldes et webhooks.'
---
...
Pour les clés API, les URL de base et les limites, voir **[Authentification](/reference/setup/authentication)**. Pour votre première requête authentifiée, voir **[Intégration API](/reference/setup/integration)**.
...
Pour le fonctionnement des soldes live après complétion, voir **[Soldes et règlement](/reference/platform/balance-and-settlement)**.
...
Voir **[Erreurs](/reference/reference/errors)** pour la forme des échecs dans les réponses API.
...
Avec **[lomi Payment Elements](/reference/payments/lomi-payment-elements)** ou l’API Payment Intents : créez un payment intent avec **`lomi_sk_test_…`**, montez l’UI carte avec **`lomi_pk_test_…`**, puis confirmez avec le **`client_secret`** retourné. Utilisez les numéros de test ci-dessus dans le champ carte.
...
2. Saisir un MSISDN valide pour le pays (format international).
...
Les virements **Wave** vers un tiers exigent `recipient.name` et `recipient.phone` avec une **clé live**. Le numéro n’a pas besoin de correspondre à un `payout_method_id`. Voir **[Virements](/reference/payments/payouts)**.
...
Utilisez le `client_secret` avec **`lomi_pk_test_…`** côté client. Voir **[Encaissements directs](/reference/payments/charges)** et **[lomi Payment Elements](/reference/payments/lomi-payment-elements)**.

</Tab>

<Tab value="Lien de paiement">

Créez des liens en mode **Test** dans le tableau de bord, ou via l’API avec votre clé de test pour `environment: test`. Voir **[Liens de paiement](/reference/payments/payment-links)**.
...
Utilisez des clés API **test** et les flux de [Paiements en bac à sable](/reference/setup/sandbox-payments).

1. **Créez un produit récurrent** avec `trial_enabled: true` ou `first_payment_type: non_initial` (voir [Produits](/reference/payments/products)).
2. **Lien de paiement ou session** — ouvrez le tunnel hébergé ou l’URL S’abonner de la vitrine avec le `product_id` récurrent.
3. **Essai + Stripe** — utilisez `4242 4242 4242 4242` ; le tunnel enregistre la carte via SetupIntent avec **0** dû aujourd’hui. Attendez **`SUBSCRIPTION_CREATED`** à la fin de l’inscription.
...
1. Dans le tableau de bord (**Developers → Webhooks**), ajoutez un point de terminaison en mode **Test**.
2. Utilisez le **secret de signature** de ce point de terminaison pour vérifier la signature sur le **corps brut** de la requête.
...
Voir **[Webhooks](/reference/payments/webhooks)**. Pour l’automatisation, voir le **[Guide de tests](/core/advanced-guides/testing)**.
...
| Erreurs bac à sable MTN | MSISDN ou identifiants invalides | Numéros documentés par MTN pour votre pays |
| Pas d’e-mail client | Normal en test | Notifications désactivées en test |

## Documentation associée

- **[Authentification](/reference/setup/authentication)** — clés, environnements, sécurité
- **[Intégration API](/reference/setup/integration)** — première requête et index API
- **[Sessions checkout](/reference/payments/checkout-sessions)** — API checkout hébergé
- **[Encaissements directs](/reference/payments/charges)** — Wave, MTN et carte (`POST /charge/card`)
- **[Liens de paiement](/reference/payments/payment-links)** — liens réutilisables
- **[lomi Payment Elements](/reference/payments/lomi-payment-elements)** — UI carte côté client
- **[Webhooks](/reference/payments/webhooks)** — abonnements aux événements
- **[Soldes et règlement](/reference/platform/balance-and-settlement)** — règles des soldes live
- **[Guide de tests](/core/advanced-guides/testing)** — automatisation, CI, écouteurs webhook

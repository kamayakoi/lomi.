...
<Mermaid
  chart={`flowchart TD
  apiKey["Clé API test ou live"] --> env["environment : test ou live"]
  link["Lien de paiement ou session checkout"] --> env
  env --> ledger["Solde test vs comptes live"]
  env --> method{"Moyen de paiement"}
  method --> cards["Paiements carte"]
  method --> wave["Wave"]
  method --> mtn["MTN MoMo"]`}
/>

...
```mermaid
flowchart TD
  apiKey[Cle_API_test_ou_live] --> env[environment_test_ou_live]
  link[Lien_ou_session_checkout] --> env
  env --> ledger[Solde_test_vs_comptes_live]
  env --> method{Moyen_de_paiement}
  method --> cards[Paiements_carte]
  method --> wave[Wave]
  method --> mtn[MTN_MoMo]
```
...
| **API Payment Intents** | `POST /payment-intents` avec `lomi_sk_test_…` → confirmation avec `lomi_pk_test_…` côté client | Même logique : en attente jusqu’à confirmation, puis completed et solde de test crédité |
...
Utilisez le `client_secret` avec **`lomi_pk_test_…`** côté client. Voir **[Payment Intents](/reference/payments/payment-intents)** et **[lomi Payment Elements](/reference/payments/lomi-payment-elements)**.
...
| Clé de test mais checkout « live » | Lien créé en mode **Live** | Recréer le lien en **Test** |
...
- **[Payment intents](/reference/payments/payment-intents)** — paiements carte intégrés

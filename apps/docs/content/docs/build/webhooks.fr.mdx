...
**Livraison sortante :** les requêtes de **lomi. vers votre URL** ne s’authentifient pas avec `` `Authorization` `` / `` `X-API-Key` `` — seule la signature compte. Un middleware qui impose Bearer ou clé API sur **tous** les POST renverra souvent `` `401` `` ([détails](/build/advanced-guides/handling-webhooks#webhook-signing-vs-authorization)).

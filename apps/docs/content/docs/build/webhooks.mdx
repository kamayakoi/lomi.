...
Roughly speaking: **`4xx` client errors are usually terminal** for that automated sequence (misconfiguration should be corrected on your side), while **transient failures** (`5xx`, timeouts, certain network faults) may be retried up to a ceiling that depends on whether the webhook was dispatched **synchronously** or through the **queued** worker/outbox pipeline. Older documentation sometimes overstated calendar duration; treat the authoritative breakdown as **[Webhook reliability — Timeouts and retries](/build/advanced-guides/webhook-reliability#timeouts-retries-and-client-errors)**, where we document the approximate **~4 second** outbound HTTP timer, **four** synchronous attempts at most on retryable errors, **five** queue attempts where Bull is in play, and the meaning of **`lomi_environment`**.

...
Non-terminal subscription changes use a Stripe-style envelope: current state in `object`, what changed in `previous_attributes`, and optional `context` for debugging.

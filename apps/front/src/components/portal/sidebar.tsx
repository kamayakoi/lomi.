...
import { useSidelinks, type SidebarItem } from '@/lib/data/sidelinks.tsx'
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 flex h-16 items-center justify-between border-b border-[hsl(var(--sidebar-border))]/40 bg-[hsl(var(--sidebar-background))] px-4 md:hidden">
        <a href="#" className="group flex items-center gap-2.5 transition-transform duration-200 ease-in-out hover:scale-[0.98]">
          ...
          <div className="flex flex-col justify-center gap-0.25">
            ...
            <span className='text-[10px] font-semibold text-emerald-500'>{t('portal.brand.control_portal')}</span>
          </div>
        </a>
      ...
      >
        ...
        <Layout fixed className={cn(navOpened ? 'h-full' : '', 'bg-[hsl(var(--sidebar-background))] rounded-br-xl rounded-tr-xl flex-1 flex flex-col')}>
          ...
          {/* Remove the duplicate mobile header and start directly with Navigation links */}
          <Nav
            id='sidebar-menu'
            className={cn(

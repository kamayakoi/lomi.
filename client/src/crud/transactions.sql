...
BEGIN
  ...
  IF p_end_customer_id IS NULL OR p_payment_method_id IS NULL OR p_organization_id IS NULL OR 
     ...
     p_currency_id IS NULL OR p_status IS NULL OR p_transaction_type IS NULL OR p_payment_info IS NULL THEN
  ...
  INSERT INTO transactions (
    ...
    amount, fee_amount, fee_id, currency_id, status, transaction_type,
    payment_info, metadata
  )
  VALUES (
    p_end_customer_id, p_payment_method_id, p_organization_id, p_user_id,
    p_amount, p_fee_amount, p_fee_id, p_currency_id, p_status, p_transaction_type,
...
CREATE OR REPLACE FUNCTION get_transaction_by_id(p_transaction_id BIGINT)
RETURNS transactions AS $$
